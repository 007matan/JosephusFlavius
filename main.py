...
class JosephusGUI:
    def __init__(self, master):
        ...
        right_frame = tk.Frame(root, width=200, height=700)
        ...
        self.label_number_ninjas = tk.Label(right_frame, text="Number of Ninjas:")
        self.label_number_ninjas.grid(row=2, column=0)
        ...
        self.label_jump = tk.Label(right_frame, text="Jump Size:")
        self.label_jump.grid(row=4, column=0)

        self.jump_size = tk.Scale(right_frame, from_=1, to=60, tickinterval=10, orient='horizontal', length=250)
        self.jump_size.grid(row=5, column=0)

        self.label_survivors = tk.Label(right_frame, text="Number of survivors:")
        self.label_survivors.grid(row=6, column=0)

        self.number_survivors = tk.Scale(right_frame, from_=1, to=60, tickinterval=10, orient='horizontal', length=250)
        self.number_survivors.grid(row=7, column=0)

        self.label_start_ninja = tk.Label(right_frame, text="Ninja to start with:")
        self.label_start_ninja.grid(row=8, column=0)

        self.start_soldier = tk.Scale(right_frame, from_=1, to=60, tickinterval=10, orient='horizontal', length=250)
        self.start_soldier.grid(row=9, column=0)

        self.label_speed = tk.Label(right_frame, text="Speed:")
        self.label_speed.grid(row=10, column=0)

        self.speed = tk.Scale(right_frame, from_=1, to=3, tickinterval=1, orient='horizontal', length=250)
        self.speed.grid(row=11, column=0)

        self.radio_dir = tk.Checkbutton(right_frame, text='counter clockwise', variable=self.radio_var)
        self.radio_dir.grid(row=12, column=0)
        self.radio_dir.pack
        ...
        left_frame.grid(row=0, column=1, padx=50, pady=28)
    ...
    def update(self,s):
        ...
        self.canvas.itemconfig(soldier_id, image=self.image_weapon)
    ...
    def start(self):
        ...
        image = tk.PhotoImage(file="ninja.png")
        ...
        self.image_weapon = tk.PhotoImage(file="big_shuriken.png")
        ...
        # Show the winner
        if len(self.l) > 1:
            self.canvas.create_text(x0, y0, text="Survivors: " + str(sorted(self.l)), font=("Arial", 24), fill='white',
                                    width=r)
        else:
            self.canvas.create_text(x0, y0, text="Survivor: " + str(sorted(self.l)), font=("Arial", 24), fill='white',
                                    width=r)
